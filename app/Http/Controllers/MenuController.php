...
class MenuController extends BaseController
{
    ...
    private function buildMenuTree($menuItems): array
    {
        ...
        // Create a lookup map that groups menu items by their parent ID
        foreach ($menuItems as $item) {
            $parentId = $item->parent_id;
            if (!isset($itemsByParentId[$parentId])) {
                $itemsByParentId[$parentId] = [];
            }
            $itemsByParentId[$parentId][] = $item;
        }

        // Helper function to get children for a specific parent ID
        $getChildren = function ($parentId) use (&$getChildren, &$itemsByParentId) {
            if (!isset($itemsByParentId[$parentId])) {
                return [];
            }

            return array_map(function ($item) use (&$getChildren) {
                $item->children = $getChildren($item->id);
                return $item;
            }, $itemsByParentId[$parentId]);
        };

        // Build the tree by getting the top-level items (parent_id is null)
